...
const ROIDS_JAG = .3 // jaggedness of asteroids (0 = none, 1 = max)
...
function createAsteroidBelt() {
    ...
    for (let i = 0; i < ROIDS_NUM; i++) {
        ...
        } while (distBetweenPoints(ship.x, ship.y, x, y) < ROIDS_SIZE * 2 + ship.rad);
...
function update() {
    ...
    for (let i = 0; i < roids.length; i++) {
        ...
        if (x > canvas.width + rad) { // use shorthand local x variable for comparison
            roids[i].x = 0 - rad;
        } else if (x < 0 - rad) {
